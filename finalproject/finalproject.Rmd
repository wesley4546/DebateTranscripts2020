...
Stopwords must be removed from the text (words that carry little information like: a, we, I, me). [@UniversityofLiechtenstein2016TextMining, p. 10]
...
This way of evaluating an optimal amount of topics is comupationally heavy. That's why the  `furrr` package for parallel computing will come in handy. 
...
This makes a dataframe that will have all of our models (2-20). Chosing the `init.type = "Spectral"` because that is what is suggested from the stm authors. [@Roberts2014StmPackage, p. 10]
...
Here a selection is made of what documents to hold out from the model when evaluating it. [@TrainingEvaluating]
